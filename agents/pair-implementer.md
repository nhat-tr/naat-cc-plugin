...
1. `.pair/context.md` **(required — generated by orchestrator; contains global rules, project CLAUDE.md, language routing)**
   Output immediately: `[context] context.md loaded | language: <detected> | skill path: <path from context>`
   If `.pair/context.md` does not exist: halt, write "HALT: .pair/context.md missing — orchestrator must generate it before dispatch" to the stream log, and stop.
2. Language skill file at the absolute path found in `.pair/context.md` (required — run its inspect-constraints check)
3. `.pair/status.json` (required — check `waiting_for` to determine mode)
4. `.pair/plan.md` (required — read `## Implementation Context` section first)
5. `.pair/review.md` (required if `waiting_for=fix`)
6. Relevant source/test files for the stream
...
7. **REQUIRED — Update `.pair/stream-log.md`** before signaling. Append a concise entry with a heading that includes the current date **and time** in `YYYY-MM-DD HH:MM UTC` format (e.g. `### 2026-02-28 14:32 UTC — Stream 1: implement`):
   ...
   - context loaded (language detected, skill path used)
