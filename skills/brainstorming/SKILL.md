...
At activation, before opening the Visual Companion or letting reconnaissance materially grow the conversation, register the Agent Conversation for the Freshness Gate. This is the mandatory first action: run `pair-loop --register-brainstorming --runtime auto`. It needs no stdin, registers the native Codex `CODEX_THREAD_ID` or Claude `CLAUDE_CODE_SESSION_ID` Agent Conversation, and seeds a bounded bootstrap checkpoint so that a conversation which later goes cold is still intercepted. The Stop hook enriches an incomplete bootstrap from the exact provider transcript; sealing refuses an incomplete bootstrap when that recovery source is unavailable. The installed hooks also auto-register this Agent Conversation the first time the Visual Companion runs, so protection does not depend on remembering the command. Registration is idempotent and never clobbers an existing checkpoint.

Enrich that checkpoint after every material research or decision boundary and before asking the next question: invoke `pair-loop --brainstorm-checkpoint --runtime auto` with one JSON checkpoint object on stdin. The input contract is strict: unknown top-level or nested fields fail instead of being silently discarded. Wrong field types, conflicting camel-case/snake-case aliases, and malformed digests also fail instead of being coerced. Include bounded finding statements with evidence references and digests. Use `coreAnchor` for the confirmed Core Anchor; `findings` entries shaped as `{ "finding": "...", "reference": "path:line", "digest": "<sha256>" }`; `confirmedChoices`; `rejectedAlternatives`; `currentDirection`; `unresolvedDecisions`; `nextAction`; and `artifacts` entries shaped as `{ "path": "repo/relative/file", "sha256": "<sha256>" }`. Keep the stable fields cumulative so a later checkpoint does not drop an earlier confirmed choice. When authoring this explicit checkpoint, never persist a raw prompt, transcript, private reasoning, compact summary, environment map, credential, capability token, or secret-like value. Pass only that semantic JSON object on stdin.

Interactive TTY stdin is rejected before reading. Supply the semantic JSON in the same shell invocation with a here-document, or redirect a saved checkpoint file; never start the checkpoint command and paste or stream JSON afterward.

The explicit semantic checkpoint remains authoritative for the stable Core Anchor, confirmed choices, and rejected alternatives. After later transcript progress, automatic Stop recovery refreshes the volatile current direction, unresolved decisions, and next action while retaining bounded assistant conclusions and repository artifacts. It also fills a missing Core Anchor without replacing a manually reviewed one.
