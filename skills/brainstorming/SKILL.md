...
The normal visual path has at most five model-visible command boundaries: one reconnaissance batch, one `scaffold` or Architecture `present`, one `publish` when the scaffold path requires it, one background feedback wait per review round, and one `reply` or revision. Do not call `visual-session.cjs --help` on the normal path, do not reread a generated scaffold before editing known fields, and do not run status/drain probes between these steps. Do not poll the retained server execution handle; keep its session identifier and leave it running until shutdown.
