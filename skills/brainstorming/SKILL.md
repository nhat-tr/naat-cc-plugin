...
At activation, before opening the Visual Companion or letting reconnaissance materially grow the conversation, run the mandatory first action: `pair-loop --register-brainstorming --runtime auto`. It needs no stdin, registers the native Codex `CODEX_THREAD_ID` or Claude `CLAUDE_CODE_SESSION_ID` Agent Conversation for the Freshness Gate, and seeds a bounded bootstrap checkpoint so a conversation that later goes cold is still intercepted. Registration is idempotent and never clobbers an existing checkpoint; the installed hooks also auto-register on first Visual Companion use, and the Stop hook enriches an incomplete bootstrap from the exact provider transcript.

Refresh the checkpoint after every material research or decision boundary and before asking the next question: invoke `pair-loop --brainstorm-checkpoint --runtime auto` with one JSON object on stdin, supplied in the same shell invocation via here-document or file redirect (interactive TTY stdin is rejected). Fields: `coreAnchor` (the confirmed Core Anchor as one string); `findings` entries `{ "finding": "...", "reference": "path:line", "digest": "<sha256>" }`; `confirmedChoices`; `rejectedAlternatives`; `currentDirection`; `unresolvedDecisions`; `nextAction`; `artifacts` entries `{ "path": "repo/relative/file", "sha256": "<sha256>" }`.

**Send deltas, not restatements.** Stable fields — `coreAnchor`, `findings`, `confirmedChoices`, `rejectedAlternatives`, `artifacts` — merge cumulatively engine-side, so include only new or changed entries. Volatile fields — `currentDirection`, `unresolvedDecisions`, `nextAction` — replace the previous value when provided; an explicitly empty `unresolvedDecisions` list marks the decisions resolved. The input contract is strict: unknown fields, wrong types, conflicting camel-case/snake-case aliases, and malformed digests fail instead of being coerced. Never persist a raw prompt, transcript, private reasoning, compact summary, environment map, credential, capability token, or secret-like value.

The explicit semantic checkpoint remains authoritative for the stable Core Anchor, confirmed choices, and rejected alternatives. Automatic Stop recovery refreshes the volatile fields from later transcript progress and fills a missing Core Anchor without replacing a manually reviewed one. An artifact that changes on disk after checkpointing is settled at seal time into a bounded drift finding — it never blocks the handover, so pinning evidence digests is safe even when implementation follows.

The Freshness Gate blocks a Cold Agent Conversation before model processing. Continue through its exact Agent Conversation Handover in a plain fresh provider-affine conversation: `pair-loop --fresh-from <handover-id> --runtime auto`, then `pair-loop --adopt-handover <handover-id> --runtime codex|claude`. Do not resume or fork the old conversation. The only old-conversation recovery is `pair-loop --allow-cold-resume <handover-id> --once --confirm-cost-risk`; its next Stop boundary refreshes and seals the checkpoint before retiring the source. Direct adoption also retires the source and continues in the adopter.
...
- State whether it satisfies the FULL Core Anchor scope or only the illustrative example; never rank an example-only fix above a scope-matching one unless the user narrowed the anchor.
...
Every acceptance criterion must have a stable ID and a matching verification entry. Keep the acceptance criterion itself outcome-only; put the proof under its matching `### AC-<n>` verification heading. Do not leave TODO/TBD placeholders. Write the specification once after approval; fold later corrections in with targeted edits, never a full rewrite per round.
...
Choose exactly one Workspace Kind from the user's decision: Product Concept Studio for comparable UI concepts, Architecture Canvas for topology and ownership, Research Evidence Board for sourced claims and unknowns, Business Reasoning Canvas for actors/outcomes/experiments, Feature Review Workbench for approved intent against implementation evidence, or UML Diagram for a standard UML view (component, state machine, activity, or sequence). Architecture Canvas and UML Diagram share a Draft fast-path (a compact Draft compiled and served directly with `present --draft`): read only `references/architecture-visual.md` or `references/uml-visual.md` respectively on the normal path, and do not load `visual-companion.md`, schemas, or generated assets unless recovery is required. Exception: at session start, if the reference being used (or `visual-companion.md`) has a modification time newer than the session's last checkpoint, skim its header/changelog before proceeding — capability upgrades are otherwise invisible on the normal path and go unused until the user asks why. For the other Workspace Kinds, read only the relevant range of `visual-companion.md`. The reusable Visual Shell owns HTML, layout, annotation, chat history, and stable Component rendering.

Run routine Visual Session commands inside the active sandbox. Do not set `require_escalated` proactively for scratch reads or writes, `scaffold`, `validate`, `present`, `publish`, `wait`, `drain`, or `reply`; first invoke the command normally and let the configured writable roots and network policy apply. If a command is genuinely denied and interactive approvals are enabled, request one scoped approval for the `node <skill-dir>/scripts/visual-session.cjs` executable prefix. Never request approval separately for `scaffold`, `validate`, `publish`, `wait`, `drain`, and `reply` in the same Visual Session.

The normal visual path has at most five model-visible command boundaries: one reconnaissance batch, one `scaffold` or Draft authoring, one `validate` followed immediately by the first `present`, one background feedback wait per review round, and one `publish` plus `reply` per revision. Always `validate` the authored document or Draft before the first `present` — it runs the same compiler and render preflight without a server, so an authoring slip costs one cheap retry instead of a failed present. **The first `present` becomes the server process and does not exit: launch it through the harness's background-command mechanism and retain the handle** (a blocking foreground `present` dies at the shell command timeout); later `present`/`publish` calls reuse the live session and return immediately in the foreground. Do not call `visual-session.cjs --help` on the normal path, do not reread a generated scaffold before editing known fields, and do not run status/drain probes between these steps. Do not poll the retained server execution handle; keep its session identifier and leave it running until shutdown.
