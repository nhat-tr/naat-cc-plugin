...
function handleCompletedImplementation(root, state, context, slice, projected, output, options, dependencies, correction) {
  ...
  if (verification.status !== 0) {
    // Both of these run before the red gate because neither is one. A check somebody killed says nothing
    // about the code, so it takes the road the provider's own interrupt takes: no correction, no block, the
    // attempt's edits left where they are. Reaching for the keyboard must never cost the slice the one
    // correction it was saving for a real defect — and it must never start the retry below, which would
    // relaunch the very check the person just stopped.
    if (verification.human_interrupt) return markAttemptInterrupted(root, state, projected);
    // Refusing to guess is not a failure to implement either: the check never ran, so there is no failure to
    // reproduce, no signature to compare, and nothing a retry could make progress against.
...
function handleRedGate(root, state, context, slice, projected, options, dependencies, { correction, verification }) {
  ...
  if (stalled || attempts > budget) {
    ...
    // Carries `dependencies` because this write is terminal, and teardown now hangs off the write: without it
    // stopRuntime resolves the real command instead of the one this run was given, so a Work that exhausts its
    // deterministic budget leaves its program running. The retry above is the only other new road to a block,
    // and it is the road a failing probe takes — the exact case where something is definitely still up.
    return saveState(root, state, dependencies);
  }
  projected.status = 'correction-ready';
...
function verifyActiveSlice(root, options = {}, dependencies = {}) {
  ...
  //
  // A runtime refusal is the one park that neither half of that test can see. Its verification was green and
  // what stopped it was the host, so it records no verification_failure, and the human's fix is usually to
  // stop a program rather than to edit a file — leaving the tree unchanged too. Re-running the whole gate,
  // tests and then the question the program is asked, is exactly the evidence that decides whether the
  // refusal still stands, so it clears on its own terms.
  ...
  const clearsDeterministicFailure = clearsRefusal
    || (parked && (changedPaths(state.worktree).length > 0 || Boolean(projected.verification_failure)));
