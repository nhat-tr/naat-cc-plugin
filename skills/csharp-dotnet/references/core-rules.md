...
Full rationale for the condensed rules in `SKILL.md` § 2 (Core Rules) and § 3 (Code Style) — the "why," not just the "what." Read the matching heading here when a rule's intent isn't obvious from its one-line form. For runtime code samples, see `code-examples.md`; this file is rules and rationale, not code.
...
Full rationale for the § 3 Code Style bullets in `SKILL.md` (braces/control-flow style has its own worked example in `code-examples.md` § Braces / Control-Flow Style; member ordering and Modern C# feature gating have their own sections below).
...
Full member ordering (top to bottom):
