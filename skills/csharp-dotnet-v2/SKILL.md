...
- **Prefer `DbContext` directly for simple queries.** Extract a named query class (e.g., `OrderQueries`) only when the same complex query appears in 2+ services or when scattering queries across services hurts readability.
...
- **Map explicitly** using static methods or extensions on the target type. Explicit mapping is searchable, debuggable, and fails at compile time when shapes change:
...
- `references/code-examples.md` — longer code samples for patterns described above. No rules — just code.
